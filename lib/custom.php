...
//Office Hours - Pre-submission
add_action('gform_pre_submission_5', 'add_guests');
function add_guests($form) {
  global $wpdb;
	$formid = 5;
	$participant_1 = rgpost( 'input_15');
	$participant_2 = rgpost( 'input_19');
	$participant_3 = rgpost( 'input_23');
	$participant_4 = rgpost( 'input_27');
	$participant_5 = rgpost( 'input_31');
	$participant_6 = rgpost( 'input_35');

	if (empty($participant_1)){
		$_POST['input_43'] = 1;
	} elseif (empty($participant_2)){
		$_POST['input_43'] = 2;
	} elseif (empty($participant_3)){
		$_POST['input_43'] = 3;
	} elseif (empty($participant_4)){
		$_POST['input_43'] = 4;
	} elseif (empty($participant_5)){
		$_POST['input_43'] = 5;
	} elseif (empty($participant_6)){
		$_POST['input_43'] = 6;
	} else {
		$_POST['input_43'] = 7;
	}
...
//Gravity Forms
add_filter('gform_field_value_org_code', 'populate_organization_code');
function populate_organization_code($value)
{
	$current_user = wp_get_current_user();
    if (get_the_author_meta('organization_code', $current_user->ID)):
    return get_the_author_meta('organization_code', $current_user->ID);
    endif;
}

add_filter('gform_field_value_org_name', 'populate_organization_name');
function populate_organization_name($value)
{
	$current_user = wp_get_current_user();
    if (get_the_author_meta('organization', $current_user->ID)):
    return get_the_author_meta('organization', $current_user->ID);
    endif;
}

// template parts as a function 
function template_parts_function() {
	// begin output buffering
    ob_start();

    // output some text
    $zeet = get_template_part('templates/content', 'access-denied');
    printf($zeet);
...
// template parts function as a shortcode
function register_shortcodes()
{
    add_shortcode('recent-posts', 'template_parts_function');
}
